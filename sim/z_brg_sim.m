% This function assigns Spice model parameters from struct generated by z_sim_gen_params().
% Then it generates parasitic couplings based on setup from 'sim.m'.
% Then it calls Spice to do the calculation, reads results, does post processing
% and returns some results. 
%
% Usage:
%   [model, Z2,d_Z,d_ph,d_Rs,d_Xs] = z_brg_sim(model, par, f, swp, cfg)
%
% Params:
...
%     ...
%   par - model parameters defined in 'sim.m' (follow code, there too many of them...)
%   f - frequency of this sweep step
%   swp - sweep configuration from 'sim.m'
%     swp.f - freq list
%     swp.Iac - nominal current (use 1)
%     swp.Idc - DC current (not used now)
%   cfg.mode - '4TP' or '2x4T' mode strings
%
% Returns:
%   model - modified simulation session (do not loose at least for pipe mode!)
%   Z2 - simulated measurement of Z2 value
%   d_Z - relative deviation of Z2 modulus (simulation - expected)
%   d_ph - absolute deviation of Z2 phase (simulation - expected)
%   d_Rs - absolute deviation of Z2 real (simulation - expected)
%   d_Xs - absolute deviation of Z2 imag (simulation - expected)
%
